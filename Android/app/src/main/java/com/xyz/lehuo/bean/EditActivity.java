...
import static com.xyz.lehuo.bean.Note.SAVE_SUCCESS;
    ...
    private void initView() {
        ...
        saveButton.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                try {
                    FileOutputStream outStream = new FileOutputStream("/sdcard/"+newTitle+".txt", true);
                    ...
                    try {
                        ...
                        outputStream = openFileOutput(Constant.NOTE_LIST_FILE_NAME, Context.MODE_APPEND);
                        OutputStreamWriter nameWriter = new OutputStreamWriter(outStream);
                        nameWriter.write(newTitle);
                        nameWriter.close();
                        outputStream.close();
                    }catch(Exception e) {
                        throw e;
                ...
                if (Conf.isLogin) {
                    ...
                    params.add(new BasicNameValuePair("note_title", newTitle));
                    params.add(new BasicNameValuePair("uid", user.getUid()));
                    params.add(new BasicNameValuePair("nbid", (user.getNotebooks()).get(0)));
                    params.add(new BasicNameValuePair("note_title", newTitle));
                    params.add(new BasicNameValuePair("note_date", newDate));
                    params.add(new BasicNameValuePair("note_week", newWeek));
                    params.add(new BasicNameValuePair("note_content", newContent));
                        ...
                        public void onSuccess(String result) {
                            try {
                                ...
                                if (jsonObject.getInt("code") == 1) {
                                    ...
                                    Note note = new Note();
                                    note.setId(object.getString("nid"));
                                    note.setNbid(object.getString("nbid"));
                                    note.setNoteName(object.getString("note_title"));
                                    note.setDate(object.getString("note_date"));
                                    note.setWeek(object.getString("note_week"));
                                    note.setContent(object.getString("note_content"));

                                    //user.setUid(object.getString("_id").substring(9, object.getString("_id").length() - 2));

                                    Note.save(EditActivity.this, note);
