...
public class ClipZoomImageView extends ImageView implements
	...
	private class AutoScaleRunnable implements Runnable {
		static final float BIGGER = 1.3f;
		static final float SMALLER = 0.7f;
		...
		public void run() {
			...
			if (((tmpScale > 1f) && (currentScale < mTargetScale))
				...
				ClipZoomImageView.this.postDelayed(this, 10);
			} else
			// 设置为目标的缩放比例
			{
	...
	public Bitmap clip() {
		...
		Bitmap scaleBitmap = Bitmap.createBitmap(bitmap, mHorizontalPadding,
				mVerticalPadding, getWidth() - 2 * mHorizontalPadding,
				getWidth() - 2 * mHorizontalPadding);

		return getRoundBitmap(scaleBitmap);
	}
	...
	public static Bitmap getRoundBitmap(Bitmap bitmap) {
		...
		paint.setXfermode(new PorterDuffXfermode(Mode.SRC_IN)); // 画图像
		paint.setAlpha(255);
...
/*	public Bitmap toRoundCorner(Bitmap bitmap) {
		...
		DLog.i(TAG, "Math.max(bitmap.getWidth(),bitmap.getHeight())/2.0f: " + Math.max(bitmap.getWidth(),
				bitmap.getHeight()) / 2.0f);
